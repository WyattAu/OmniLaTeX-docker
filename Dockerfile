...
ARG TL_INSTALL_SHA256=""
ARG EISVOGEL_SHA256=""
ARG OMNILATEX_CLS_SHA256=""
ARG USER_ID="1000"
ARG GROUP_ID="1000"
...
RUN --mount=type=cache,target=/var/cache/apt,id=apt-cache-${TARGETARCH},sharing=locked \
    ...
    # Required for full LaTeX toolchain support (CI and dev builds)
...
RUN if [ "$RUNTIME_USER" = "root" ]; then mkdir -p /root/.pandoc/templates/ && cp /home/${USER}/.pandoc/templates/eisvogel.latex /root/.pandoc/templates/; fi
...
RUN mkdir -p /home/${USER}/texmf/tex/latex && \
    ...
    chown --recursive "$( [ "$RUNTIME_USER" = "root" ] && echo "root:root" || echo "${USER}:${USER}" )" /home/${USER}/ && \
...
ARG SET_ENTRYPOINT="true"
ENV SET_ENTRYPOINT=${SET_ENTRYPOINT}
